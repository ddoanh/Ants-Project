...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while current_place.is_hive == False:
            if current_place.bees and step >= self.min_range and step <= self.max_range:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.chew_countdown > 0:
            ...
            if self.place.bees:
                random = random_bee(self.place.bees)
                random.reduce_health(random.health)
...
# END Problem 12
    ...
    def __init__(self, gamestate):
        ...
        if gamestate.keep_track > 0:
            raise DuplicateQueensException
        gamestate.keep_track += 1 
        # END Problem 12
    ...
    def action(self, gamestate):
        ...
        while curr_place != None:
            if curr_place.ant != None:
                if curr_place.ant.is_container:
                    if curr_place.ant.ant_contained != None and curr_place.ant.ant_contained.buffed == False:
                        curr_place.ant.ant_contained.buff()
                        curr_place.ant.ant_contained.buffed = True
                    if curr_place.ant.buffed == False:
                        curr_place.ant.buff()
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.blocked():
            ...
            if self.slowed_turn != None and self.slowed_turn > 0:    
                if gamestate.time % 2 == 0:
                    if self.scared_turn != None and self.scared_turn > 0:
                        self.move_to(self.place.entrance)
                        self.scared_turn -= 1
                    else:
                        self.move_to(destination)
            elif self.scared_turn != None and self.scared_turn > 0:
